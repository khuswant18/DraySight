...
const memoryStore: MemStore = {
  runs: new Map<string, TrackingRun>(),
};
...
function initSqlite() {
  ...
  try {
    ...
    console.warn("[Storage] Using in-memory fallback store:", err);
    useMemoryStore = true;
    return null;
  }
}
  ...
  if (db) {
    try {
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, 'RUNNING', ?, 0, 0, ?, ?)
      `);
      stmt.run(id, portal, containers.length, JSON.stringify(containers), startedAt);
    } catch (e) {
      memoryStore.runs.set(id, initialRun);
    }
  } else {
    memoryStore.runs.set(id, initialRun);
  }
...
export function saveContainerResult(runId: string, result: ContainerResult) {
  ...
  if (db) {
    try {
      ...
      console.error("[Storage] SQLite save error, syncing to memory:", e);
    }
  }
...
export function completeRun(runId: string, status: RunStatus = "COMPLETED") {
  ...
  if (db) {
    try {
      ...
      console.error("[Storage] SQLite complete error:", e);
    }
  }
...
export function getRun(runId: string): TrackingRun | null {
  ...
  if (db) {
    try {
      ...
      console.error("[Storage] SQLite getRun error, checking memory:", e);
    }
  }
...
export function listRecentRuns(limit: number = 20): TrackingRun[] {
  ...
  if (db) {
    try {
      ...
      console.error("[Storage] SQLite listRecentRuns error, checking memory:", e);
    }
  }
